...
let (max_coords : galaxy list -> int * int) =
    List.fold_left (fun (accx, accy) galaxy ->
        (Int.max accx galaxy.x, Int.max accy galaxy.y)
    ) (0, 0)
;;
...
let expand_universe (offset : int) (orig_galaxies : galaxy list) : galaxy list =
    ...
    let rec aux_horizontal xoff i galaxies : galaxy list =
        if Int.equal i (max_coords_x + 1) then []
        else match (galaxies_by_col i galaxies) with
    ...
    let rec aux_vertical yoff i galaxies : galaxy list =
        if Int.equal i (max_coords_y + 1) then []
        else match (galaxies_by_row i galaxies) with
