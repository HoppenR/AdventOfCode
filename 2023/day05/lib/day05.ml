...
let (get_seed_number : int -> int RangeMap.t list -> int) =
  List.fold_left (fun seed product_map ->
    RangeMap.fold
      (fun (r : range) (target : int) (acc : int) ->
        if r.start <= seed && seed <= r.stop then seed + target else acc)
      product_map
      seed)
;;
...
let lowest_seed_number (maps : int RangeMap.t list) (seeds : int list) : int =
  List.fold_left
    (fun min seed -> Int.min min (get_seed_number seed maps))
    Int.max_num
    seeds
;;

let (find_lowest_start : range list -> int) =
  List.fold_left (fun (acc : int) (r : range) -> min acc r.start) Int.max_num
;;
...
let get_new_ranges (r : range) (in_range : range) (change : int) : range list =
  ...
  then (
    ...
    let range2 =
      { start = overlap_start + change; stop = overlap_stop + change }
    in
    let range3 = { start = overlap_stop + 1; stop = r.stop } in
    if r.start < overlap_start && r.stop > overlap_stop
    then [ range1; range2; range3 ]
    else if r.start < overlap_start
    then [ range1; range2 ]
    else if r.stop > overlap_stop
    then [ range2; range3 ]
    else [ range2 ])
  else [ r ]
;;
...
let find_output_mappings (maps : int RangeMap.t list) (seeds : int list) =
  let rec seed_ranges = function
    | [] -> []
    | start :: span :: tl ->
      { start; stop = start + span - 1 } :: seed_ranges tl
    | _ -> failwith "unreachable"
  in
  List.map
    (fun (seed_range : range) ->
      List.fold_left
        (fun acc product_map ->
          RangeMap.fold
            (fun (in_range : range) (change : int) (acc : range list) ->
              List.concat_map
                (fun (r : range) -> get_new_ranges r in_range change)
                acc)
            product_map
            acc)
        [ seed_range ]
...
let (parse : string list -> int list * int RangeMap.t list) =
  let extract_header_seeds input =
    ...
    | [ _; last ] ->
      Str.split (Str.regexp_string " ") last |> List.map Int.of_string
    | _ -> failwith "unreachable"
  in
  let build_map =
    List.map (fun (product_map : string) ->
      List.fold_left
        (fun (acc : int RangeMap.t) (line : string) ->
