...
let filter_num (extnum : bool) (digit : cdigit) : int option =
...
let sum_digit_line (lines : cdigit list list) (extnum : bool) : int =
  List.fold_left
    (fun acc digits ->
      ...
      let remaining = List.filter_map (filter_num extnum) digits in
        ...
        (fun (offset, cur) ->
          let find_pfx (substr, _) =
            try
              String.starts_with
                substr
                (String.sub line offset (String.length substr))
            with
            | Invalid_argument _ -> false
          in
          if is_digit cur
          then Some (Literal (String.make 1 cur |> Int.of_string))
          else (
