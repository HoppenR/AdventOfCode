...
type pipe = Horizontal | Ground | NE | NW | SE | SW | Start | Vertical

let (pipe_from_char : char -> pipe) =
    function
    | '-' -> Horizontal
    | '.' -> Ground
    | '7' -> NW
...
let (find_start_pos : pipe CoordMap.t -> coord) =
    CoordMap.to_seq
    %> Seq.find (function (_, Start) -> true | _ -> false)
    %> function
    | Some (c, _) -> c
    | None -> failwith "unreachable"
;;

let step_one (prev : coord) (cur : coord) : (pipe -> coord) =
    function
    | Horizontal -> if prev.x < cur.x
        then { cur with x = cur.x + 1 }
        else { cur with x = cur.x - 1 }
    | Ground -> failwith "unreachable"
    | NE -> if Int.equal prev.x cur.x
        then { cur with x = cur.x + 1 }
        else { cur with y = cur.y + 1 }
    | NW -> if Int.equal prev.x cur.x
        then { cur with x = cur.x - 1 }
        else { cur with y = cur.y + 1 }
    | SE -> if Int.equal prev.x cur.x
        then { cur with x = cur.x + 1 }
        else { cur with y = cur.y - 1 }
    | SW -> if Int.equal prev.x cur.x
        then { cur with x = cur.x - 1 }
        else { cur with y = cur.y - 1 }
    | Start -> { cur with y = cur.y + 1 }
    | Vertical -> if prev.y < cur.y
...
let (parse : string list -> pipe CoordMap.t) =
    let aux pipe_map y line =
        String.to_seq line
        |> Seq.fold_lefti (fun acc x ch ->
            CoordMap.add {x; y} (pipe_from_char ch) acc
        ) pipe_map
    in
